...
#    respective csv
# 5. If count discrimination is needed, this program requires columns titled 
#    'CP1', 'CP2', 'CP3', 'CE'
...
def get_model(data, state):
    train, test = train_test(data, test_size=TEST, random_state=state)
...
def get_linreg_models(peps, bind, count):
    p_n, b_n = format_data(discrim(peps, count)), format_data(discrim(bind, count))
    p_TRAIN, p_TEST, b_n = train_test_split(p_n, b_n, 0)
...
if __name__ == "__main__":
    ...
    # step 3: yes/no do you want count discrimination?:
    count_discrim = input("3. Perform count discrimination? [y/n]: ")
    
    if count_discrim:
        # step 3.1; if yes, up to what count sum do you want to discrim?
        highest_count = input("3.1. What is highest count discrim?: ")
        
        # step 3.2: what is the interval between count discrims?
        COUNTS = np.arange(0, highest_count, input("3.2. What is interval between count discrims?: "))
        
    # step 4: what is the name of the "Sequence" column in the csv?
    SEQ = input("4. What is the name of the column containing peptide sequences?: ")
    
    # step 5: what is the name of the column containing binding affinity data?
    TARGET = input("5. What is the name of column containing binding affinity data?: ")
    
    # step 6: how many cross validations do you wish to perform?
    NUM_CV = input("6. How many cross validations do you wish to perform?: ")
    
    # step 7: what is upper binding affinity limit of "weak" binding peptides?
    WEAK_UPPER = input("7: what is upper binding affinity limit of \"weak\" binding peptides?: ")
    
    # step 8: what is the upper binding affinity limit of "medium" binding peptides?
    MED_UPPER = input("8: what is upper binding affinity limit of \"medium\" binding peptides?: ")
    
    # step 9: what is the upper binding affinity limit of strong binding peptides?
    STRONG_UPPER = input("9: what is upper binding affinity limit of strong binding peptides?: ")
